...
router.get("/email/appeared",function(req,res){
  if (authenticated == false) {
    res.redirect("/login");
  } else {
  res.render("email_appeared");
  }
});

router.get("/email/round2",function(req,res){
  if (authenticated == false) {
    res.redirect("/login");
  } else {
  res.render("email_round2");
  }
});

router.get("/email/selected",function(req,res){
  if (authenticated == false) {
    res.redirect("/login");
  } else {
  res.render("email_selected");
  }
});

router.get("/email/onlyapplied",function(req,res){
  if (authenticated == false) {
    res.redirect("/login");
  } else {
  res.render("email_onlyapplied");
  }
});



router.post("/email/all",function(req,res){

  var subject = req.body.subject;
  var text = req.body.text;
  var getQuery = "select email from registers ";
  con.query(getQuery,function(err,result){
    if(err) throw err;
  ...
  res.render("email_succesful");
  })
});

router.post("/email/appeared",function(req,res){
  var subject = req.body.subject;
  var text = req.body.text;
  var getQuery = "select email from registers where status = 'Appeared' ";
  con.query(getQuery,function(err,result){
  if(err) throw err;
...
router.post("/email/round2",function(req,res){
  var subject = req.body.subject;
  var text = req.body.text;
  var getQuery = "select email from registers where status = 'selected for round 2' ";
  con.query(getQuery,function(err,result){
  if(err) throw err;
...
router.post("/email/selected",function(req,res){
    var subject = req.body.subject;
  var text = req.body.text;
  var getQuery = "select email from registers where status = 'Selected' ";
    con.query(getQuery,function(err,result){
  if(err) throw err;
...
router.post("/email/onlyapplied",function(req,res){
    var subject = req.body.subject;
  var text = req.body.text;
  var getQuery = "select * from registers where status = 'Only Applied' ";
    con.query(getQuery,function(err,result){
  if(err) throw err;
